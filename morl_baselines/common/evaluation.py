...
def eval_mo_reward_conditioned(
    ...
    while not done:
        if render:
            env.render()
        obs, r, terminated, truncated, _ = env.step(agent.eval(obs, disc_vec_return))
        done = terminated or truncated
        vec_return += r
        disc_vec_return += gamma * r
        gamma *= agent.gamma
...
def policy_evaluation_mo(agent, env, w: np.ndarray, rep: int = 5) -> Tuple[float, float, np.ndarray, np.ndarray]:
    ...
    avg_disc_vec_return = np.round_(avg_disc_vec_return, decimals=2)  #if env != dst
...
def log_all_multi_policy_metrics(
    ...
    if ref_front is not None:
        ...
        ref_front = np.round_(ref_front, decimals=2)  #if env != dst

        recall = find_in(ref_front,current_front)
        precision = find_in(current_front,ref_front)
...
def find_in(A, B, base=0):
    ...
    for a in A:
        for b in B:
            if base == 0:
              if np.linalg.norm(a - b, ord=1) < 0.30*np.linalg.norm(a):
                  cnt += 1.0
                  break
            elif base == 1:
              if np.linalg.norm(a - b, ord=1) < 0.30*np.linalg.norm(b):
                  cnt += 1.0
                  break
            elif base == 2:
