...
class MOQLearning(MOPolicy, MOAgent):
    ...
    def update(self):
        ...
        if self.log and self.global_step % 4000 == 0:
            wandb.log(
                {
                    f"charts/epsilon": self.epsilon,
                    f"losses/scalarized_td_error": self.scalarization(td_error, self.weights),
                    f"losses/mean_td_error": np.mean(td_error),
        ...
        for _ in range(1, total_timesteps + 1):
            ...
            if self.terminated or self.truncated:
                ...
                if self.log and self.global_step % 4000 == 0:
                    wandb.log(
                        {
                            f"charts/SPS": int(self.global_step / (time.time() - start_time)),
                            "global_step": self.global_step,
                        },
                    )
                    if "episode" in info:
