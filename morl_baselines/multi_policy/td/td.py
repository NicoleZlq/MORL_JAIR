...
from morl_baselines.multi_policy.linear_support.td_support import TDSupport
...
class morl_td(MOPolicy, MOAgent):
    """GPI-PD Algorithm.
    ...
    def __init__(
        ...
        self.ideal_point = th.tensor(ideal_point).float().to(self.device)
        self.use_td = use_td
        ...
        self.log = log
        if self.log:
            self.setup_wandb(project_name, experiment_name, wandb_entity, seed)
    ...
    def _rollout_dynamics(self, w: th.Tensor):
        ...
        for iteration in range(num_times):
            ...
            for h in range(self.dynamics_rollout_len):
                ...
                if self.use_td:
                    q_values = th.einsum("r,bar->bar", w, self.ideal_point-psi_values)
                    max_q1,a = th.max(q_values, dim=2)
                    max_q1 =max_q1.view(obs.size(0), len(self.weight_support), self.action_dim)
                    max_q, ac = th.min(max_q1, dim=2)
                    pi = th.argmin(max_q, dim=1)
                else:
    ...
    def update(self, weight: th.Tensor):
        ...
        for g in range(self.gradient_updates if self.global_step >= self.dynamics_rollout_starts else 1):
            ...
            with th.no_grad():
                ...
                if self.use_td:
                    scalarized_next_q_values = th.einsum("nbar,br->nbar", self.ideal_point-next_q_values, w)  # q_i(s', a, w)
                    scalarized_next_q_values,a = th.max(scalarized_next_q_values,dim=3)
                    min_inds = th.argmax(scalarized_next_q_values, dim=0)
                ...
                if self.use_td:
                  max_q = th.einsum("br,bar->bar", w, self.ideal_point - next_q_values)

                  min_value, a = th.max(max_q, 2)

                  max_acts = th.argmin(min_value, dim=1)
                else:
            ...
            if self.log and self.global_step % 10000 == 0:
                wandb.log(
            ...
            if self.per or self.gpi_pd:
                if self.gpi_pd:
                    ...
                    if self.use_td:
                        gper = th.einsum("br,br->br", w[: len(idxes)], gtd_error).abs()
                        gper,a = th.max(gper, 1)
                    else: 
                ...
                if self.per:
                    ...
                    if self.use_td:
                        per = th.einsum("br,br->br", w[: len(idxes)], td_error).abs()
                        per,a = th.max(per,1)    
                    else:
    ...
    def gpi_action(self, obs: th.Tensor, w: th.Tensor, return_policy_index=False, include_w=False):
        ...
        if self.use_td:
          q_values_th = th.einsum("r,bar->bar", w, self.ideal_point-q_values)

          min_value1, a = th.max(q_values_th, dim=2)
          min_value, a = th.min(min_value1, dim=1)  # max_i max_a q(s,a,w_i)
          policy_index = th.argmin(min_value)
          action = a[policy_index].detach().item()

        else:
          scalar_q_values = th.einsum("r,bar->ba", w, q_values)
    ...
    def max_action(self, obs: th.Tensor, w: th.Tensor) -> int:
        ...
        if self.use_td:
          q = th.einsum("r,bar->bar", w, self.ideal_point - psi)
          min_value, a = th.max(q, dim =2)
          max_act = th.argmin(min_value, dim=1)
    ...
    def _envelope_target(self, obs: th.Tensor, w: th.Tensor, sampled_w: th.Tensor):
        ...
        if self.use_td:
            q_values = th.einsum("br,nbpar->nbpar", w, self.ideal_point-next_q_target)

            q_values,a = th.max(q_values,dim=4)

            min_inds = th.argmax(q_values, dim=0)
        ...
        if self.use_td:
          q_values = th.einsum("br,bpar->bpar", w, self.ideal_point - next_q_target)
          max_q1, ac = th.max(q_values, dim=3)
          max_q, ac = th.min(max_q1,dim=2)
          pi = th.argmin(max_q, dim=1)
    ...
    def train(
        ...
        if self.log:
            self.register_additional_config({"ref_point": ref_point.tolist(), "known_front": known_pareto_front,"num_eval_weights_for_front": num_eval_weights_for_front})
        max_iter = total_timesteps // timesteps_per_iter

        known_pareto_front = np.multiply(known_pareto_front, 0.99)
        
        import csv
        f = open('222.csv','w')
        writer = csv.writer(f)
        for i in known_pareto_front:
            writer.writerow(i)
        f.close()
        
        if self.use_td:
            linear_support = TDSupport(num_objectives=self.reward_dim, epsilon=0.0 if weight_selection_algo == "ols" else None, ideal_point = np.array(self.ideal_point.cpu()))
        else:  
        ...
        for iter in range(1, max_iter + 1):
            ...
            if weight_selection_algo == "gpi-ls":
                M = linear_support.get_weight_support() + linear_support.get_corner_weights(top_k=10) + [w]
