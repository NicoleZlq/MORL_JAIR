...
class TD(MOPolicy, MOAgent):
    ...
    def max_action(self, obs: th.Tensor, w: th.Tensor) -> int:
       ...
       # scalarized_q_values = th.einsum("r,bar->ba", w, q_values)
        scalarized_q_values= th.einsum("r,bar->ba", w, self.ideal_point - q_values)
        max_act = th.argmax(scalarized_q_values, dim=1)

        #HQ = th.div(self.ideal_point - q_values, w)

       # max_act = th.argmin(th.max(HQ,1)[0])
    #    action = int(action)
    ...
    def envelope_target(self, obs: th.Tensor, w: th.Tensor, sampled_w: th.Tensor) -> th.Tensor:
        ...
        scalarized_next_q_values = th.einsum("br,bwar->bwa", w, self.ideal_point - next_q_values)
    ...
    def ddqn_target(self, obs: th.Tensor, w: th.Tensor) -> th.Tensor:
        ...
        td_q_values = th.div(self.ideal_point - q_values, w[0])

        #scalarized_q_values = th.einsum("br,bar->ba", w, q_values)
        max_acts = th.argmin(th.max(td_q_values,2)[0],dim=1)
    ...
    def train(
        ...
        num_episodes = 0
        self.ideal_point = th.tensor(ideal_point).to(self.device)
        ...
        for _ in range(1, total_timesteps + 1):
            ...
            if eval_env is not None and self.log and self.global_step % 10000 == 0:
                current_front = [
