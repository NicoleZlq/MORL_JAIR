...
class TDContinuousAction(MOAgent, MOPolicy):
    ...
    def update(self, weight: th.Tensor):
        ...
        for _ in range(self.gradient_updates):
            ...
            with th.no_grad():
                ...
                if self.use_td:
                  scalarized_q_targets = th.einsum("nbr,br->nbr", self.ideal_point -q_targets, w)
                  scalarized_next_q_values,a = th.max(scalarized_q_targets,dim=2)
 
                  inds = th.argmax(scalarized_next_q_values, dim=0, keepdim=True)
            ...
            if self.per:
                ...
                if self.use_td:
                    per = th.einsum("br,br->br", w[: len(idxes)], per).abs()
                    per,a = th.max(per,1)  
    ...
    def eval(
        ...
        if self.use_gpi:
            ...
            if self.use_td:
              q_values = th.einsum("par,r->par", self.ideal_point-values, w)
   
              max_q, a = th.max(q_values, dim=2)
              min_q, a = th.min(max_q,dim=1)
              policy_index = th.argmin(min_q)  # max_i max_a q(s,a,w_i)
