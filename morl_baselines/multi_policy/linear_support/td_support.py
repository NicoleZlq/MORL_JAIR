...
class TDSupport:
    ...
    def __init__(
        ...
        self.corner = equally_spaced_weights(self.num_objectives, n=100)
        self.corner = [np.array(w) for w in self.corner]
    ...
    def gpi_ls_priority(self, w: np.ndarray, gpi_expanded_set: List[np.ndarray]) -> float:
        ...
        def best_vector(values, w):
            ...
            for i in range(1, len(values)):
                if np.max((self.ideal_point-values[i])* w ) < np.max((self.ideal_point-minmax_v)* w ):
                    minmax_v = values[i]
            return minmax_v

        min_value_ccs = self.max_ccs_value(w)
        min_value_gpi = best_vector(gpi_expanded_set, w)
        #min_value_gpi = np.min((self.ideal_point-min_value_gpi)* w )
        #priority =  min_value_ccs - min_value_gpi
        priority = np.linalg.norm(min_value_ccs-min_value_gpi,ord=2)
    ...
    def max_scalarized_value(self, w: np.ndarray) -> Optional[float]:
        ...
        v_ccs = 0
        for v in self.ccs:
            diff =   w * (self.ideal_point - v)
            minmax_ = np.min(diff)
            if minmax_ < td_minmax:
                td_minmax = minmax_
                v_ccs = v
        return td_minmax
    
    def max_ccs_value(self, w: np.ndarray) -> Optional[float]:
        """Returns the maximum scalarized value for weight vector w.
    ...
    def remove_obsolete_weights(self, new_value: np.ndarray) -> List[np.ndarray]:
        ...
        for i, (priority, cw) in enumerate(self.queue):
            if np.min((self.ideal_point-new_value) * cw) < self.max_scalarized_value(cw):
                W_del.append(cw)
    ...
    def remove_obsolete_values(self, value: np.ndarray) -> List[int]:
        ...
        for i in reversed(range(len(self.ccs))):
            ...
            #     if np.min((self.ideal_point-self.ccs[i]) * w) == self.max_scalarized_value(w) and np.min((self.ideal_point-value) * w)> np.min((self.ideal_point-self.ccs[i]) * w) 
                
            # ]
            count = 0
            for j in range(len(value)):
                if value[j]>= self.ccs[i][j]:
                    count+=1
            if count == j+1 :
    ...
    def is_dominated(self, value: np.ndarray) -> bool:
        ...
        if len(self.ccs) == 0:
            return False
        new_ccs = [np.array(i) for i in self.ccs]

        value = np.array(value)

        new_ccs.append(value)
        
        new_ccs = filter_pareto_dominated(new_ccs)

        if np.any(np.all(value == new_ccs, axis=1)):
            return False
        # if value in new_ccs:
        #     return False
        # for w in self.visited_weights:
        #     diff = w * (self.ideal_point - value)
        #     max_ = np.min(diff)
        #     if max_ <= self.max_scalarized_value(w):
        #         return False
        return True
...
if __name__ == "__main__":
    ...
    num_objectives = 3
    ols = TDSupport(num_objectives=num_objectives, epsilon=0.0001, verbose=True)
    while not ols.ended():
        w = ols.next_weight()
        print("w:", w)
        value = _solve(w)
