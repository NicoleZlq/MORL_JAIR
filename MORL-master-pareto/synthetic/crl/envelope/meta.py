...
sys.path.append('E:\\UMstudy\\python\\RL\\MORL-master\\MORL-master-pareto\\synthetic\\crl')
...
class MetaAgent(object):
    ...
    def act(self, state, preference):
        ...
        Q = torch.mv(Q.data, preference)     #矩阵相乘
    ...
    def memorize(self, state_np, action, next_state_np, reward, terminal):
        ...
        state = torch.from_numpy(state_np).type(FloatTensor)
        ...
        domain_value = preference * q


        self.if_domain, deque_list = Pareto.get_pareto_frontier(domain_value, self.length)

        if self.if_domain:

            self.trans_mem.append(self.trans(
                torch.from_numpy(state_np).type(FloatTensor),  # state
                action,  # action
                torch.from_numpy(next_state_np).type(FloatTensor),  # next state
                torch.from_numpy(reward).type(FloatTensor),  # reward
                terminal))  # terminal
        if len(deque_list):
            new_mem = deque()
            for i in range(len(deque_list)):
                new_mem.append(self.trans_mem[i])
            self.trans_mem = new_mem
        print(len(self.trans_mem),"trans_mem")



        if not terminal:
            next_state = torch.from_numpy(next_state_np).type(FloatTensor)
    ...
    def nontmlinds(self, terminal_batch):
        ...
        terminal = inds[mask.eq(1)]
        inds = inds[mask.eq(0)]

        return inds , terminal
    ...
    def learn(self):
        if len(self.trans_mem) > self.batch_size:
            ...
            minibatch = self.sample(self.trans_mem, self.batch_size)
            ...
            nontmlmask, terminal = self.nontmlinds(terminal_batch)
            #***********************************
